...
**Cambio v1.2:** Fase 2 reescrita. Auth simplificado para operador único: `CredentialsProvider` con `email + password` verificado contra env vars `ADMIN_EMAIL` + `ADMIN_PASSWORD_HASH`. Sin magic link, sin Resend en auth, sin tablas `Account`/`VerificationToken` activas. Cero cambios en Fases 0 y 1.
...
| 2   | Autenticación                     | CredentialsProvider, operador único, contraseña via env vars, protección por capa | AC-07               |
...
SIGUIENTE_TAREA: Fase 2 — Auth.js v5 con CredentialsProvider (operador único), bcryptjs, protección de rutas por capa (sin middleware.ts)
```
...
Configurar Auth.js v5 con `CredentialsProvider` para **operador único**: email + contraseña verificados contra env vars `ADMIN_EMAIL` y `ADMIN_PASSWORD_HASH`. Sin magic link, sin Resend en auth, sin dependencia de tablas `Account`/`VerificationToken`. Protección de rutas por capa sin `middleware.ts`.

**Por qué env vars y no DB:** el sistema tiene un único operador. Almacenar credenciales en env vars es el patrón más simple, sin superficie de ataque adicional, y sin requerir seed de DB ni UI de gestión de usuarios.
...
MODELOS_AUTH: User, Account, Session, VerificationToken — en schema (creados en Fase 1)
              NOTA: Account y VerificationToken quedan en schema pero sin uso activo.
              Session SÍ se usa — strategy: database requiere esta tabla.
              User SÍ se usa — se crea automáticamente en primer login si no existe.
PATRON_AUTH: auth() directo en cada capa — NO middleware.ts
DEPS_REQUERIDAS: next-auth@beta, @auth/prisma-adapter, bcryptjs — ya instaladas en Fase 0
AUTH_VARS_NUEVAS: ADMIN_EMAIL, ADMIN_PASSWORD_HASH (añadir a .env.local y .env.local.example)
```

## Tareas

### T2.0 — Instalar bcryptjs
...
> Si ya está instalado desde Fase 0, verificar con `npm list bcryptjs` y omitir.

### T2.1 — Generar hash de contraseña (one-time setup)

El operador genera su hash antes de configurar el proyecto. Proveer un script utilitario:

```typescript
// scripts/hash-password.ts
// Uso: npx tsx scripts/hash-password.ts "mi-contraseña-segura"
// Copiar el output a ADMIN_PASSWORD_HASH en .env.local
...
### T2.2 — Actualizar `.env.local.example`

Añadir al archivo existente (las variables de Auth.js ya existentes se mantienen):

```bash
# ── Autenticación — Operador único ────────────────────────────────────────
# Generar hash con: npm run hash-password "tu-contraseña"
ADMIN_EMAIL="admin@crypto.com"
ADMIN_PASSWORD_HASH="$2a$12$..."
```

Eliminar del `.env.local.example` las variables que ya no aplican:

```bash
# Estas variables NO son necesarias con CredentialsProvider:
# RESEND_API_KEY  ← sigue necesitándose para ALERTAS (Fase 8), NO para auth
# RESEND_FROM_EMAIL ← ídem
# AUTH_TRUST_HOST  ← sigue siendo necesaria en producción Vercel
```

> **Importante:** `RESEND_API_KEY` y `RESEND_FROM_EMAIL` siguen en el proyecto — se usan en Fase 8 para las alertas de oportunidades. Solo dejan de usarse para autenticación.

### T2.3 — Configurar Auth.js v5 con CredentialsProvider
...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  providers: [
    Credentials({
      ...
      async authorize(credentials) {
        // 1. Validar shape del input con Zod
        const parsed = LoginSchema.safeParse(credentials);
        if (!parsed.success) return null;

        const { email, password } = parsed.data;

        // 2. Verificar contra env vars — sin consulta a DB para las credenciales
        const adminEmail = process.env.ADMIN_EMAIL;
        const adminHash = process.env.ADMIN_PASSWORD_HASH;

        if (!adminEmail || !adminHash) {
          console.error(
            "[auth] ADMIN_EMAIL or ADMIN_PASSWORD_HASH not configured",
          );
          return null;
        }
        ...
        // 3. Buscar o crear el User en DB (necesario para session strategy: database)
        //    El PrismaAdapter necesita un User.id para asociar la Session.
        const user = await prisma.user.upsert({
          ...
          create: {
            email: adminEmail,
            name: "Admin",
            emailVerified: new Date(), // marcado como verificado desde el inicio
          },
        });
  ...
  callbacks: {
    session({ session, user }) {
      // user.id viene del PrismaAdapter (session strategy: database)
...
**Por qué `upsert` y no `findUnique`:**
`CredentialsProvider` con `strategy: database` requiere que el objeto retornado por `authorize()` tenga un `id` que corresponda a un `User` real en la tabla. El `upsert` garantiza que el User existe en DB sin importar si es el primer login o un login posterior, sin necesidad de un script de seed separado.

### T2.4 — Extender tipos de sesión
...
### T2.5 — API Route handler de Auth.js
...
### T2.6 — Helpers de autenticación por capa

Sin cambios respecto al diseño de protección por capa. El patrón es idéntico independientemente del provider.
...
/**
 * Para Route Handlers (API Routes).
 * Retorna Response 401 si no hay sesión.
...
/**
 * Para Server Actions.
 * Retorna el userId o null — sin efectos secundarios de redirect.
 */
...
/**
 * Lectura simple sin redirect — para componentes con contenido condicional.
 */
...
### T2.7 — Guard de autenticación en el Layout del dashboard (stub)

```typescript
// app/(dashboard)/layout.tsx
// Stub para Fase 9. El guard de auth debe existir desde esta fase.
// La Fase 9 lo reemplazará con el layout completo (sidebar + header)
// manteniendo el await requireAuth().
...
> **Nota:** Auth.js v5 con `CredentialsProvider` redirige a `/login?error=CredentialsSignin` en caso de credenciales inválidas. La página lo detecta y muestra el mensaje de error apropiado.

### T2.9 — LoginForm con email + contraseña
...
### T2.10 — Página raíz: redirect según sesión
...
### T2.11 — Verificar que NO existe middleware.ts

```bash
ls middleware.ts 2>/dev/null && echo "ERROR: middleware.ts existe — eliminar" || echo "OK: middleware.ts no existe"
rm -f middleware.ts
```
...
- [ ] `npm run hash-password "test"` → imprime `ADMIN_PASSWORD_HASH=...` sin errores
- [ ] `.env.local` tiene `ADMIN_EMAIL` y `ADMIN_PASSWORD_HASH` configurados
- [ ] `GET /login` con credenciales correctas → redirige a `/dashboard`
- [ ] `GET /login` con contraseña incorrecta → muestra mensaje de error en la misma página
...
```
FASE_COMPLETADA: 2
AUTH_PROVIDER: CredentialsProvider — email + password vs ADMIN_EMAIL + ADMIN_PASSWORD_HASH (env vars)
AUTH_CONFIG: lib/auth.ts — NextAuth con PrismaAdapter + CredentialsProvider + bcryptjs
...
LOGIN_PAGE: app/(auth)/login/page.tsx — form email + password, error via searchParams
ROOT_PAGE: app/page.tsx — redirect a /dashboard o /login
SESSION_STRATEGY: database (PrismaAdapter, tabla Session activa)
TABLAS_AUTH_DB: User (activa — upsert en authorize()), Session (activa), Account/VerificationToken (en schema, sin uso activo)
MIDDLEWARE_TS: NO EXISTE
SCRIPT_UTIL: scripts/hash-password.ts — genera bcrypt hash para ADMIN_PASSWORD_HASH
AUTH_VARS_REQUERIDAS: AUTH_SECRET, AUTH_TRUST_HOST, ADMIN_EMAIL, ADMIN_PASSWORD_HASH
RESEND_VARS: siguen en proyecto — usadas en Fase 8 para alertas, NO para auth
...
3. Login con email + contraseña → redirige a `/dashboard`
...
- [ ] Login funciona con email + contraseña (CredentialsProvider)
...
AUTH: CredentialsProvider funcionando — ADMIN_EMAIL + ADMIN_PASSWORD_HASH verificados
